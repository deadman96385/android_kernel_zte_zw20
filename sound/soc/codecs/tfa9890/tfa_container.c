				      ...
				      nxpTfaBitfield_t *bf)
{
	...
	error =
	    reg_read(dev_idx, (unsigned char)(bfUni.Enum.address), &regvalue);
	if (error)
		return error;
...
int tfaContGetCalProfile(int dev_idx)
{
	...
	for (prof = 0; prof < nprof; prof++) {
		...
		if (strnstr(profile_name, ".cal", sizeof(profile_name)) != NULL) {
			cal_idx = prof;
...
int tfaContIsTapProfile(int dev_idx, int prof_idx)
{
	...
	if (strnstr(profile_name, ".tap", sizeof(profile_name)) != NULL) {
