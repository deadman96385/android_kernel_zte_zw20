...
static int get_step_chg_jeita_setting(struct step_chg_info *chip)
{
	struct device_node *node = chip->dev->of_node;
	u32 max_fv_uv, max_fcc_ua;
	int rc;
	...
	chip->step_chg_config->param.use_bms = false;
	chip->soc_based_step_chg =
		of_property_read_bool(node, "qcom,soc-based-step-chg");
	...
	chip->ocv_based_step_chg =
		of_property_read_bool(node, "qcom,ocv-based-step-chg");
	...
	rc = read_range_data_from_node(node,
			...
			max_fcc_ua);
	if (rc < 0) {
		pr_err("Read qcom,step-chg-ranges failed, rc=%d\n",
					rc);
	...
	rc = read_range_data_from_node(node,
			...
			BATT_HOT_DECIDEGREE_MAX, max_fcc_ua);
	if (rc < 0) {
		pr_err("Read qcom,jeita-fcc-ranges failed, rc=%d\n",
					rc);
	...
	if (rc < 0) {
		pr_err("Read qcom,jeita-fv-ranges failed, rc=%d\n",
					rc);
		chip->sw_jeita_cfg_valid = false;
	}
...
static void get_config_work(struct work_struct *work)
{
	...
	rc = get_step_chg_jeita_setting(chip);
...
static int handle_jeita(struct step_chg_info *chip)
{
	...
	if (0/*chip->jeita_arb_en && fv_uv > 0*/) {
		if (!rc && (pval.intval > fv_uv))
...
set_jeita_fv:
	...
	pr_info("SW jeita Voltage now = %d FCC = %duA FV = %duV\n",
			pval.intval, fcc_ua, fv_uv);
