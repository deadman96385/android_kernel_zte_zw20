...
struct policy_status_node {
	u32 status;
	char *status_name;
	int (*handle)(struct charger_policy_info *policy_info, bool is_charging);
};
...
static bool charger_policy_check_soc_reach_max(struct charger_policy_info *policy_info)
{
	...
	if (policy_info->demo_enable) {
		...
		if (capacity >= temp_cap) {
			pr_policy("capacity(%d) reach daemon max(%d)\n", capacity, temp_cap);
			return true;
		}
	}
...
static int charger_policy_status_ide(struct charger_policy_info *policy_info, bool is_charging)
{
	struct timespec ts;

	if (!is_charging) {
		if ((policy_info->charging_begin) || (policy_info->overtime_status)) {
			policy_info->policy_status = POLICY_STATUS_DISABLE;
			pr_policy("reset idle status.\n");
			}
		return 0;
	}

	/* check usb leave */
	if (charger_policy_check_usb_present(policy_info) != true) {
		policy_info->policy_status = POLICY_STATUS_DISABLE;
	...
	/*demo_enable check*/
	if (policy_info->demo_enable) {
		pr_policy("demo enabled, policy enter into running\n");
	...
	if ((ts.tv_sec - policy_info->charging_begin) >= policy_info->overtime_seconds) {
		pr_policy("overtime found, policy enter into running\n");
		charger_policy_disable_cas(policy_info, true);
...
static int charger_policy_status_runchging(struct charger_policy_info *policy_info, bool is_charging)
{
	/* check usb leave */
	if (charger_policy_check_usb_present(policy_info) != true) {
		policy_info->policy_status = POLICY_STATUS_DISABLE;
...
static int charger_policy_status_rundischging(struct charger_policy_info *policy_info, bool is_charging)
{
	struct timespec ts;
...
static int charger_policy_status_forcedischging(struct charger_policy_info *policy_info, bool is_charging)
{
	struct timespec ts;
...
static int charger_policy_status_disable(struct charger_policy_info *policy_info, bool is_charging)
{
	charger_policy_ctrl_charging_enable(policy_info, true, true);
...
static int charger_policy_notifier_handler(struct charger_policy_info *policy_info)
{
	...
	policy_info->policy_status = (policy_info->policy_status > POLICY_STATUS_DISABLE) ?
					POLICY_STATUS_DISABLE : policy_info->policy_status;
	...
	if (policy_status_list[policy_info->policy_status].handle)
		policy_status_list[policy_info->policy_status].handle(policy_info, is_charging);
...
static void charger_policy_ctrl_timer(struct charger_policy_info *policy_info)
{
	if (charger_policy_check_usb_present(policy_info) == true)
		...
		unsigned long event, void *data)
{
	...
	if ((strcmp(name, policy_info->config_prop.battery_phy_name) == 0)
			|| (strcmp(name, "usb") == 0)) {
		/* pr_policy("Notify, %s update status\n", name); */
		charger_policy_ctrl_timer(policy_info);
...
int charger_policy_demo_sts_set(const char *val, const void *arg)
{
	...
	if (sscanf(val, "%d", &enable) != 1) {
		pr_policy("enter inval cmd\n");
		return -EINVAL;
	}
...
int charger_policy_expired_sec_set(const char *val, const void *arg)
{
	...
	if (sscanf(val, "%d", &sec) < 1) {
		pr_policy("enter inval sec\n");
		return -EINVAL;
	}
...
int charger_policy_max_cap_set(const char *val, const void *arg)
{
	struct charger_policy_info *policy_info = (struct charger_policy_info *) arg;
	int cap = 0;
	...
	if (sscanf(val, "%d", &cap) < 1) {
		pr_policy("enter inval cap\n");
		return -EINVAL;
	}
	...
	pr_policy("Set capacity_max = %d\n", cap);

	return 0;
}

int charger_policy_max_cap_get(char *val, const void *arg)
{
	struct charger_policy_info *policy_info = (struct charger_policy_info *) arg;
	...
	pr_policy("Get demo_capacity_max = %d\n",
				policy_info->config_prop.demo_capacity_max);

	return snprintf(val, PAGE_SIZE, "%u", policy_info->config_prop.demo_capacity_max);
}

int charger_policy_min_cap_set(const char *val, const void *arg)
{
	struct charger_policy_info *policy_info = (struct charger_policy_info *) arg;
	int cap = 0;
	...
	pr_policy("Set capacity_min = %d\n", cap);

	return 0;
}

int charger_policy_min_cap_get(char *val, const void *arg)
{
	struct charger_policy_info *policy_info = (struct charger_policy_info *) arg;
	...
	pr_policy("Get demo_capacity_min = %d\n",
				policy_info->config_prop.demo_capacity_min);

	return snprintf(val, PAGE_SIZE, "%u", policy_info->config_prop.demo_capacity_min);
}
...
struct zte_misc_ops policy_cap_max_node = {
	...
	.set = charger_policy_max_cap_set,
	.get = charger_policy_max_cap_get,
	.free = NULL,
	.arg = NULL,
};
...
struct zte_misc_ops policy_cap_min_node = {
	...
	.set = charger_policy_min_cap_set,
	.get = charger_policy_min_cap_get,
	.free = NULL,
	.arg = NULL,
};
				...
				const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_AUTHENTIC:
		...
		if (!pval->intval) {
			...
			policy_info->overtime_enable = false;
			policy_info->policy_status = POLICY_STATUS_DISABLE;
		} else {
			policy_info->overtime_enable = true;
		}
		break;
	default:
		pr_err("unsupported property %d\n", psp);
