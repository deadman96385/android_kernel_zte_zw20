	...
	struct configuration *configuration)
{
	...
	if (tx_num != dad->si->sensing_conf_data.tx_num) {
		dev_err(dev, "%s: TX number mismatch, expect %d, input %d!\n",
			__func__, dad->si->sensing_conf_data.tx_num, tx_num);
		ret = -EINVAL;
	}

	if (rx_num != dad->si->sensing_conf_data.rx_num) {
		dev_err(dev, "%s: RX number mismatch, expect %d, input %d!\n",
			__func__, dad->si->sensing_conf_data.rx_num, rx_num);
		ret = -EINVAL;
	}
...
static int cyttsp5_cmcp_malloc_check(struct cyttsp5_device_access_data *dad);
		...
		struct device_attribute *attr, char *buf)
{
	...
	rc = cyttsp5_cmcp_malloc_check(dad);
	if (!rc) {
		cyp_err("%s no alloc\n", __func__);
		goto exit;
	}
...
static int cyttsp5_cmcp_malloc_check(struct cyttsp5_device_access_data *dad)
{
	...
	if (dad->configs && dad->test_field_array && dad->test_search_array) {
		ret = 1;
	} else {
		cyp_err("%s no malloc configs=%p test_field_array=%p test_search_array =%p\n",
			__func__, dad->configs, dad->test_field_array, dad->test_search_array);
	}

	return ret;
}

static ssize_t cyttsp5_cmcp_malloc_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_device_access_data *dad
		= cyttsp5_get_device_access_data(dev);
	...
	val = cyttsp5_cmcp_malloc_check(dad);

	len = snprintf(buf, CY_MAX_PRBUF_SIZE, "%d\n", val);

	return len;
}

static void cyttsp5_cmcp_free(struct cyttsp5_device_access_data *dad)
{
	kfree(dad->test_search_array);
	dad->test_search_array = NULL;
	kfree(dad->test_field_array);
	dad->test_field_array = NULL;
	kfree(dad->configs);
	dad->configs = NULL;
}

static int cyttsp5_cmcp_malloc(struct cyttsp5_device_access_data *dad)
{
	int ret = 0;

	if (dad->test_search_array == NULL) {
		dad->test_search_array =
			kzalloc(sizeof(struct test_case_search) * MAX_CASE_NUM, GFP_KERNEL);
		if (!dad->test_search_array) {
			ret = -ENOMEM;
			cyp_err("%s malloc_search_array_failed\n", __func__);
			goto malloc_search_array_failed;
		}
	}

	if (dad->test_field_array == NULL) {
		dad->test_field_array =
			kzalloc(sizeof(struct test_case_field) * MAX_CASE_NUM, GFP_KERNEL);
		if (!dad->test_field_array) {
			ret = -ENOMEM;
			cyp_err("%s malloc_field_array_failed\n", __func__);
			goto malloc_field_array_failed;
		}
	}

	if (dad->configs == NULL) {
		dad->configs =
			kzalloc(sizeof(struct configuration), GFP_KERNEL);
		if (!dad->configs) {
			ret = -ENOMEM;
			cyp_err("%s malloc_configs_failed\n", __func__);
			goto malloc_configs_failed;
		}
	}

	return 0;

malloc_configs_failed:
malloc_field_array_failed:
malloc_search_array_failed:
	cyttsp5_cmcp_free(dad);

	return ret;
}

static ssize_t cyttsp5_cmcp_malloc_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct cyttsp5_device_access_data *dad
		= cyttsp5_get_device_access_data(dev);
	...
	if (ret) {
		cyp_err("%s invalid param:%s", __func__, buf);
		return -EIO;
	}
...
static DEVICE_ATTR(cmcp_malloc, S_IRUSR | S_IWUSR,
	cyttsp5_cmcp_malloc_show, cyttsp5_cmcp_malloc_store);
...
int result_save(struct device *dev, char *buf,
	...
	int no_builtin_file, int mode)
{
	u8 *out_buf = NULL;
	int index = 0;
	int byte_left;

	out_buf = vmalloc(MAX_BUF_LEN);
	if (out_buf == NULL) {
	...
	if (mode == 0) {
		...
		byte_left = strlcpy(buf, out_buf, count);
	}
	vfree(out_buf);
	return byte_left;
}
		...
		char __user *buf, size_t count, loff_t *ppos)
{
	...
	ret = cyttsp5_cmcp_malloc_check(dad);
	if (!ret) {
		cyp_err("%s no alloc\n", __func__);
		return 0;
	}
	...
	if (test_executed)
		...
		ret = result_save(dev, buf, configuration, result, cmcp_info,
			ppos, count, test_item, no_builtin_file, 0);
	else {
		char warning_info[] =
...
static int cyttsp5_setup_sysfs(struct device *dev)
{
	...
	rc = device_create_file(dev, &dev_attr_cmcp_malloc);
	if (rc) {
		dev_err(dev, "%s: Error, could not create cmcp_malloc\n",
				__func__);
...
unregister_cmcp_malloc:
	device_remove_file(dev, &dev_attr_cmcp_malloc);
...
#ifdef CYP_SELFTEST_CONFIG
int cyttsp5_selftest_memory_check(struct device *dev)
{
...
int cyttsp5_selftest_memory_alloc(struct device *dev)
{
	struct cyttsp5_device_access_data *dad = cyttsp5_get_device_access_data(dev);
	int ret = 0;

	ret = cyttsp5_cmcp_malloc(dad);

	return ret;
}

void cyttsp_selftest_memory_free(struct device *dev)
{
	struct cyttsp5_device_access_data *dad = cyttsp5_get_device_access_data(dev);

	cyttsp5_cmcp_free(dad);
}
int cyttsp5_read_thresholdfile(struct device *dev)
{
	const struct firmware *thresholdfile_fw = NULL;
	struct cyttsp5_device_access_data *dad = cyttsp5_get_device_access_data(dev);
	int ret = 0;

	cyp_info("%s filename=%s", __func__, CYTTSP5_SELFTEST_FILE);
	mutex_lock(&dad->cmcp_threshold_lock);

	ret = request_firmware(&thresholdfile_fw, CYTTSP5_SELFTEST_FILE, dev);
	if (ret != 0) {
		cyp_err("%s request_firmware failed ret=%d\n", __func__, ret);
		ret = -EINVAL;
		goto cyttsp5_request_firmware_failed;
	}

	if (!thresholdfile_fw->data || !thresholdfile_fw->size) {
		cyp_err("%s: No firmware data\n", __func__);
		ret = -ENODATA;
		goto cyttsp5_no_data;
	}

	dad->cmcp_threshold_data = kzalloc(thresholdfile_fw->size, GFP_KERNEL);
	if (!dad->cmcp_threshold_data) {
		mutex_unlock(&dad->cmcp_threshold_lock);
		ret = -ENOMEM;
		goto cyttsp5_kzalloc_failed;
	}
	memcpy(dad->cmcp_threshold_data, thresholdfile_fw->data, thresholdfile_fw->size);
	dad->cmcp_threshold_size = thresholdfile_fw->size;
...
cyttsp5_kzalloc_failed:
cyttsp5_no_data:
	release_firmware(thresholdfile_fw);

cyttsp5_request_firmware_failed:
	mutex_unlock(&dad->cmcp_threshold_lock);

	return ret;
}

ssize_t cyttsp5_selftest_test_store(struct device *dev,
		const char *buf, size_t size)
{
	return cyttsp5_cmcp_test_store(dev, NULL, buf, size);
}

ssize_t cyttsp5_selftest_test_show(struct device *dev, char *buf)
{
	return cyttsp5_cmcp_test_show(dev, NULL, buf);
}

ssize_t cmcp_results_read(struct device *dev,
		char *buf, size_t count)
{
	...
	test_item = cyttsp5_cmcp_get_test_item(dad->cmcp_test_items);
	if (dad->builtin_cmcp_threshold_status < 0) {
		cyp_err("%s: No cmcp threshold file.\n", __func__);
		no_builtin_file = 1;
	}
	mutex_unlock(&dad->sysfs_lock);
	...
	if (test_executed)
		/*save result to buf*/
		ret = result_save(dev, buf, configuration, result, cmcp_info,
			0, count, test_item, no_builtin_file, 1);
	else {
		char warning_info[] =
		"No test result available!\n";
		cyp_err("%s: No test result available!\n", __func__);

		ret = strlcpy(buf, warning_info, count);
	}
