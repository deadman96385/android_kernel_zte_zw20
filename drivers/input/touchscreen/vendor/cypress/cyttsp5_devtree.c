...
#define PINCTRL_STATE_INIT "pmx_ts_init"
static int ts_pinctrl_init(struct device *adap_dev, struct cyttsp5_core_platform_data *pdata)
{
	int retval = 0;
	...
	pdata->ts_pinctrl = devm_pinctrl_get(adap_dev);
	if (IS_ERR_OR_NULL(pdata->ts_pinctrl)) {
		retval = PTR_ERR(pdata->ts_pinctrl);
		cyp_err("Target does not use pinctrl %d\n", retval);
		goto err_pinctrl_get;
	}

	pdata->pinctrl_state_init
		= pinctrl_lookup_state(pdata->ts_pinctrl, PINCTRL_STATE_INIT);
	if (IS_ERR_OR_NULL(pdata->pinctrl_state_init)) {
		retval = PTR_ERR(pdata->pinctrl_state_init);
		cyp_err("Can not lookup %s pinstate %d\n", PINCTRL_STATE_INIT, retval);
...
static void ts_pinctrl_release(struct cyttsp5_core_platform_data *pdata)
{
	if (pdata->ts_pinctrl) {
		devm_pinctrl_put(pdata->ts_pinctrl);
		pdata->ts_pinctrl = NULL;
		...
		struct device *adap_dev, struct device_node *core_node)
{
	...
	/* add regulator */
	pdata->vdd = regulator_get(adap_dev, "vdd");
	if (IS_ERR(pdata->vdd)) {
		rc = PTR_ERR(pdata->vdd);
		pr_err("%s Regulator get failed vdd rc=%d", __func__, rc);
	}

	pdata->vcc_i2c = regulator_get(adap_dev, "vcc_i2c");
	if (IS_ERR(pdata->vcc_i2c)) {
		rc = PTR_ERR(pdata->vcc_i2c);
		pr_err("%s Regulator get failed vcc_i2c rc=%d", __func__, rc);
	}

	ts_pinctrl_init(adap_dev, pdata);
...
static void free_core_pdata(void *pdata)
{
	...
	pr_info("%s\n", __func__);
	if (core_pdata->vcc_i2c) {
		regulator_put(core_pdata->vcc_i2c);
	}
	if (core_pdata->vdd) {
		regulator_put(core_pdata->vdd);
	}

	ts_pinctrl_release(core_pdata);
