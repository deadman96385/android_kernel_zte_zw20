		...
		struct cyttsp5_touch *tch, int num_cur_tch)
{
	...
	int mt_sync_count = 0;
	u8 *tch_addr;
	static unsigned long finger_down = 0;
...
cyttsp5_get_mt_touches_pr_tch:
		if (t < sizeof(finger_down) * 8) {
			if (test_bit(t, ids) && !test_bit(t, &finger_down)) {
				__set_bit(t, &finger_down);
				cyp_info("touch down id: %d, coord [%d:%d]\n",
					t, tch->abs[CY_TCH_X], tch->abs[CY_TCH_Y]);
			} else if (!test_bit(t, ids) && test_bit(t, &finger_down)) {
				__clear_bit(t, &finger_down);
				cyp_info("touch up id: %d, coord [%d:%d]\n",
					t, tch->abs[CY_TCH_X], tch->abs[CY_TCH_Y]);
			}
		}
		parade_debug(dev, DEBUG_LEVEL_1,
		...
		struct cyttsp5_mt_data *md)
{
	...
	cyp_info("%s key_value=%d", __func__, key_value);
