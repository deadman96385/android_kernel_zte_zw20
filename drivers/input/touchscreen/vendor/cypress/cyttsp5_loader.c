...
#define CY_LDR_REQUEST_EXCLUSIVE_TIMEOUT		3000
		...
		u32 fw_ver_new, u32 fw_revctrl_new)
{
	...
	cyp_info("%s: img vers:0x%04X new vers:0x%04X\n", __func__,
			fw_ver_img, fw_ver_new);

	if (fw_ver_new > fw_ver_img) {
		cyp_info("%s: Image is newer, will upgrade\n", __func__);
		return 1;
	}

	if (fw_ver_new < fw_ver_img) {
		cyp_info("%s: Image is older, will NOT upgrade\n", __func__);
		return -EPERM;
	}

	fw_revctrl_img = ld->si->cydata.revctrl;

	cyp_info("%s: img revctrl:0x%04X new revctrl:0x%04X\n",
...
static int cyttsp5_check_config_version(struct device *dev,
		u32 cfg_ver_new)
{
	struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
	u32 cfg_ver_img;

	cfg_ver_img = ld->si->cydata.fw_ver_conf;

	cyp_info("%s: img cfg:0x%04X new cfg:0x%04X\n", __func__,
			cfg_ver_img, cfg_ver_new);

	if (cfg_ver_new > cfg_ver_img) {
		cyp_info("%s: Image config is newer, will upgrade\n", __func__);
		return 1;
	}

	cyp_info("%s: Image config is not newer, will NOT upgrade\n", __func__);
	return 0;
}
		...
		const struct firmware *fw)
{
	...
	cfg_ver_new = get_unaligned_be16(fw->data + 0x11);
	upgrade = cyttsp5_check_config_version(dev, cfg_ver_new);
	if (upgrade > 0)
		return 1;
...
static int upgrade_firmware_from_builtin(struct device *dev)
{
	...
	cyp_info("%s: Enabling firmware class loader built-in\n", __func__);
...
int cyttsp5_tp_fw_upgrade(struct tpd_classdev_t *cdev, char *fw_name, int fwname_len)
{
	char fwname[FILE_NAME_LENGTH];
	const struct firmware *cyp_fw = NULL;
	struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(cyttsp5_data->dev);
	int ret;

	if ((fwname_len <= 1) || (fwname_len >= FILE_NAME_LENGTH)) {
		cyp_err("%s fw bin name's length(%d) fail", __func__, fwname_len);
		return -EINVAL;
	}
	...
	cyp_info("%s filename=%s", __func__, fwname);

	if (ld->is_manual_upgrade_enabled) {
		cyp_err("%s last upgrade is not finished\n", __func__);
		return -EBUSY;
	}
	ld->is_manual_upgrade_enabled = 1;

	ret = request_firmware(&cyp_fw, fwname, cyttsp5_data->dev);
	if (ret != 0) {
	...
	_cyttsp5_firmware_cont(cyp_fw, cyttsp5_data->dev);
		...
		struct work_struct *fw_and_config_upgrade)
{
	...
	if (socinfo_get_nomal_flag() == 0) {
		cyp_info("%s it's not normal boot, so upgrade exits!\n", __func__);
		return;
	}
	#endif
