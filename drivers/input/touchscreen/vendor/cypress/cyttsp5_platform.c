...
static int cyttsp5_pinctrl_init(struct cyttsp5_core_platform_data *pdata)
{
	int ret = 0;
	...
	if (pdata->ts_pinctrl) {
		...
		if (ret < 0)
			cyp_err("Cannot get active pinctrl state\n");
	}

	return ret;
}

static int cyttsp5_power_source_ctrl(struct cyttsp5_core_platform_data *pdata, int enable)
{
	int rc;
	...
	if (enable) {
		if (pdata->vdd) {
			...
			if (rc)
				pr_err("%s Regulator vdd enable failed rc=%d", __func__, rc);
		}
		if (pdata->vcc_i2c) {
			rc = regulator_enable(pdata->vcc_i2c);
			if (rc)
				pr_err("%s Regulator vcc_i2c enable failed rc=%d", __func__, rc);
		}
		...
		if (pdata->vdd) {
			...
			if (rc)
				pr_err("%s Regulator vdd disable failed rc=%d", __func__, rc);
		}
		if (pdata->vcc_i2c) {
			rc = regulator_disable(pdata->vcc_i2c);
			if (rc)
				pr_err("%s Regulator vcc_i2c disable failed rc=%d", __func__, rc);
		}
	}
		...
		int on, struct device *dev)
{
	...
	if (on) {
		...
		cyttsp5_power_source_ctrl(pdata, 1);

		cyttsp5_pinctrl_init(pdata);
		...
		cyttsp5_power_source_ctrl(pdata, 0);
	}
