		...
		const u8 *data_name)
{
	...
	if (cd->show_timestamp) {
		snprintf(data_name_with_time_stamp, 100, "[%u] %s",
...
static int cyttsp5_core_sleep(struct cyttsp5_core_data *cd)
{
	...
	if (release_exclusive(cd, cd->dev) < 0)
		...
		cyp_info("%s: pass release exclusive\n", __func__);
...
static int cyttsp5_parse_input(struct cyttsp5_core_data *cd)
{
	...
	if (size == 0) {
		...
		parade_debug(cd->dev, DEBUG_LEVEL_1, "%s: error size:%X\n",
			__func__, size);
		cyttsp5_pr_buf(cd->dev, &cd->input_buf[0], CY_MAX_INPUT, "Empty Buffer");
		return 0;
	}
...
static int cyttsp5_core_wake_(struct cyttsp5_core_data *cd)
{
	...
	mutex_unlock(&cd->system_lock);
#ifdef EASYWAKE_TSG6
	cyttsp5_restore_wakegesture_flag();
#endif
...
static int cyttsp5_core_suspend(struct device *dev)
{
	...
	if (device_may_wakeup(dev)) {
		cyp_info("%s Device MAY wakeup\n", __func__);
		if (!enable_irq_wake(cd->irq))
			cd->irq_wake = 1;
	} else {
		cyp_info("%s Device MAY NOT wakeup\n", __func__);
	}
...
static void cyttsp5_resume_work_callback(struct work_struct *work)
{
		...
		unsigned long event, void *data)
{
	...
	if (*blank == FB_BLANK_UNBLANK) {
		...
		if (cd->fb_state != FB_ON) {
			ret = queue_work(cd->resume_wq, &cd->resume_work);
			if (!ret) {
				cyp_err("start resume_wq failed\n");
			}
		}
...
static void cyttsp5_setup_fb_notifier(struct cyttsp5_core_data *cd)
{
	...
	cd->resume_wq = create_singlethread_workqueue("cyttsp5_touch_resume");
	INIT_WORK(&cd->resume_work, cyttsp5_resume_work_callback);
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	if (cyttsp5_check_tp_register()) {
		cyp_info("TP have registered by other TP.\n");
		return -EPERM;
	}
	...
	/* ZTE common interface */
	cyttsp5_data = cd;
	cyttsp5_tpd_register_fw_class();
	cyttsp5_tp_register();
