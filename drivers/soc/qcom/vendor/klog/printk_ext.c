...
struct timer_t timers[TIMER_COUNT];
static bool printk_test_enabled = false;

void timer_func(unsigned long arg)
{
	struct timer_t *t = (struct timer_t *)arg;

	mod_timer(&t->timer, jiffies + HZ/10);
...
static int set_test_enabled(const char *val, const struct kernel_param *kp)
{
	...
	if (!raw_val && cur_val) {
		...
		for (i = 0; i < TIMER_COUNT; i++) {
			struct timer_t *t = &timers[i];

			t->timer_id = i;
			init_timer(&t->timer);
			t->timer.expires = jiffies + HZ/10;
...
MODULE_PARM_DESC(enabled, "enable printk test");
